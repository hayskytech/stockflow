export class DivisionsPage {
  constructor(page) {
    this.page = page
    this.addButton = page.getByRole("button", { name: /add division/i })
    this.searchInput = page.getByPlaceholder(/search divisions/i)
    this.serverError = page.locator(".alert-danger")

    this.nameInput = page.locator("#division-name")
    this.activeCheckbox = page.locator("#division-active")
    this.saveButton = page.getByRole("button", { name: /^save$/i })
    this.cancelButton = page.getByRole("button", { name: /^cancel$/i })

    this.confirmDialog = page.locator(".modal", { hasText: "Delete division?" })
    this.confirmDeleteButton = this.confirmDialog.getByRole("button", { name: /^confirm$/i })
  }

  async goto() {
    await this.page.goto("/catalog/divisions")
  }
